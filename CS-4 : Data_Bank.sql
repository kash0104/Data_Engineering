...
select count(*) from data_bank.customer_nodes  --3500
select count(*) from data_bank.customer_transactions --5868
select * from data_bank.regions  
 
  
select * from data_bank.customer_nodes WHERE EXTRACT(YEAR FROM END_DATE) = 9999
...
SELECT 
	region_id,
	percentile_disc(0.5) WITHIN GROUP (ORDER BY sum_diff) AS median,
	percentile_disc(0.8) WITHIN GROUP (ORDER BY sum_diff) AS median_80,
	percentile_disc(0.95) WITHIN GROUP (ORDER BY sum_diff) AS median_95
FROM 
	sum_diff_cte
GROUP BY
	region_id;
...
LEFT JOIN 
	monthly_balances mb
ON 
	ld.ending_month = mb.closing_month
